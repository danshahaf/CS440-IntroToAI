...
public class AlphaBetaAgent extends ChessAgent
{
	...
	private class AlphaBetaSearcher extends Object implements Callable<Pair<Move, Long> >
	{
		...
		public DFSTreeNode alphaBetaHelper(DFSTreeNode node, int depth, double alpha, double beta, boolean isMaximizingPlayer) {
	        // ----- RECRUSION BASE CASE: if leaf node ----
			if (depth == 0 || node.isTerminal()) {
	            return node;
	        }

			
			// ------ IF TREE PRUNING IS HORIZONTALLY AT MAXIMIZATION LEVEL
	        if (isMaximizingPlayer) {
	            double maximizing = Double.NEGATIVE_INFINITY; // --- SET BALANCING FACTOR
	            DFSTreeNode best_node = null;
	            for (DFSTreeNode child : node.getChildren()) { // --- FOR EACH CHILD NODE
	                DFSTreeNode current = alphaBetaHelper(child, depth - 1, alpha, beta, false);
	                double heuristic_evaluation = current.getMaxPlayerHeuristicValue(); // CALLING HEURISTIC RIGHT HERE
	                if (heuristic_evaluation > maximizing) {
	                	maximizing = heuristic_evaluation;
	                    best_node = child;
	                }
	                alpha = Math.max(alpha, heuristic_evaluation); // --- SET ALPHA
	                if (beta <= alpha) { // --- PRUNING if Beta <= Alpha
	                    break;
	                }
	            }
	            return best_node;
	        }
	        // ------ ELSE : TREE PRUNING IS HORIZONTALLY AT MINIMIZATION LEVEL
	        else {
	            double minimizing = Double.POSITIVE_INFINITY; // --- SET BALANCING FACTOR
	            DFSTreeNode best_node = null;
	            for (DFSTreeNode child : node.getChildren()) { // --- FOR EACH CHILD NODE
	                DFSTreeNode current = alphaBetaHelper(child, depth - 1, alpha, beta, true);
	                double heuristic_evaluation = current.getMaxPlayerHeuristicValue();
	                if (heuristic_evaluation < minimizing) {
	                	minimizing = heuristic_evaluation;
	                    best_node = child;
	                }
	                beta = Math.min(beta, heuristic_evaluation); // --- SET ALPHA
	                if (beta <= alpha) { // --- PRUNING if Beta <= Alpha
	                    break;
	                }
	            }
		...
		public DFSTreeNode alphaBetaSearch(DFSTreeNode node, int depth, double alpha, double beta) {
			return alphaBetaHelper(node, depth, alpha, beta, true);
		}
	 ...
	 */
	private static final int DEFAULTMAXDEPTH = 4; // NOTE: this could go up to 10 in number, but my algorithm for some reason would perform very slowly for anything over 5.
