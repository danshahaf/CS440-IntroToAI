...
public class AlphaBetaAgent extends Agent
{
	...
	private final int alphaBetaSearchDepthLimit;

	public AlphaBetaAgent(int playerNum, String[] args)
	{
		super(playerNum);
		...
		this.alphaBetaSearchDepthLimit = Integer.parseInt(args[1]);
		System.out.println("Constructed AlphaBetaAgent with depth limit [" + this.alphaBetaSearchDepthLimit + "]");
	}
	...
	public final int getAlphaBetaSearchDepthLimit() { return this.alphaBetaSearchDepthLimit; }
	...
	public Map<Integer, Action> middleStep(StateView state, HistoryView history) {
		...
		actions = this.getAlphaBetaAction(state, history);
	...
	public TreeNode alphaBetaSearch(TreeNode parentNode, int depth, double alpha, double beta)
	{
		...
		if(parentNode.getGameState().getPlayer() == GameState.FOOTMANPLAYER)
		{
			...
			{
				child.getGameState().setUtilityValue(this.alphaBetaSearch(child, depth-1, alpha, beta).getGameState().getUtilityValue());
				...
				if(child.getGameState().getUtilityValue() > beta) { return bestChild; }
		...
		} else // archer
		{
			...
			for(TreeNode child : children)
			{
				...
				if(child.getGameState().getUtilityValue() < alpha) { return bestChild; }
				else if(child.getGameState().getUtilityValue() < beta) { beta = child.getGameState().getUtilityValue(); }
			}
		}
		return bestChild;
	}

	public Map<Integer, Action> getAlphaBetaAction(StateView state, HistoryView history)
	{
		return this.alphaBetaSearch(new TreeNode(
					new GameState(state, GameState.FOOTMANPLAYER)),
				alphaBetaSearchDepthLimit,
				Double.NEGATIVE_INFINITY,
				Double.POSITIVE_INFINITY).getActions();
	}
