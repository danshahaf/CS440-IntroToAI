...
public class QAgent extends Agent
{
	...
	private HashSet<Integer> enemyUnitIds;
...
//	    this.enemyUnitIds = new HashSet<Integer>(); // Initialize enemyUnitIds as a HashSet
	...
	private Model initializeQFunction(boolean loadParams)
	{
		...
		// Layer 1
		int feature_dim = 5; // Choose the appropriate feature dimension based on your chosen features
		int hidden_dim1 = 16;
		m.add(new Dense(feature_dim, hidden_dim1, this.getRandom()));
		m.add(new Sigmoid());
		...
		// Layer 4
		int hidden_dim4 = 4;
		m.add(new Dense(hidden_dim3, hidden_dim4, this.getRandom()));
		m.add(new ReLU());
//
//		// Layer 5
		int hidden_dim5 = 4;
		m.add(new Dense(hidden_dim4, hidden_dim5, this.getRandom()));
		m.add(new Sigmoid());
//
//		// Layer 6
		int hidden_dim6 = 2;
		m.add(new Dense(hidden_dim5, hidden_dim6, this.getRandom()));
		m.add(new Tanh());
		
		// Layer 7
		int hidden_dim7 = 1;
		m.add(new Dense(hidden_dim6, hidden_dim7, this.getRandom()));
		m.add(new ReLU());
		

		// Output layer
		m.add(new Dense(hidden_dim7, 1));
		m.add(new ReLU());
     ...
     */
	// Add an instance variable to track the number of states the agent has made decisions for
	private int agentDecisionCounter = 0;

	private double getRewardForUnit(StateView state, HistoryView history, int unitId) {
	    double reward = 0.0;
	    int lastTurnNumber = state.getTurnNumber() - 1;

	    // Check for damage dealt or received
	    for (DamageLog damageLog : history.getDamageLogs(lastTurnNumber)) {
	        if (damageLog.getAttackerID() == unitId) {
	            reward += 300.0 * damageLog.getDamage(); // Positive reward for dealing damage
	        } else if (damageLog.getDefenderID() == unitId) {
	            reward -= 300.0 * damageLog.getDamage(); // Negative reward for receiving damage
	        }
	    }

	    // Check for kills or deaths
	    for (DeathLog deathLog : history.getDeathLogs(lastTurnNumber)) {
	        if (deathLog.getDeadUnitID() == unitId) {
	            reward -= 200; // Increased negative reward for getting killed
	        } else  {
	            reward += 50; // Increased positive reward for killing
	        }
	    }

	    // Check for actions started on the last turn
	    Map<Integer, Action> commandsIssued = history.getCommandsIssued(playernum, lastTurnNumber);
	    for (Map.Entry<Integer, Action> commandEntry : commandsIssued.entrySet()) {
	        if (commandEntry.getKey() == unitId) {
	            reward += 25; // Positive reward for starting an action
	        }
	    }


	    // Increment the agent decision counter
	    agentDecisionCounter++;
                                        ...
                                        int atkUnitId, int tgtUnitId)
   {
	// Define your features here
		// Example features: constant (1), distance between attacker and target, health of attacker, health of target,
		// number of allies attacking the same target, number of enemies alive
		double[] features = new double[5];

		UnitView attacker = state.getUnit(atkUnitId);
		UnitView target = state.getUnit(tgtUnitId);

		// Feature 1: Constant
		features[0] = 1;

		// Feature 2: Normalized distance between attacker and target
		int dx = attacker.getXPosition() - target.getXPosition();
		int dy = attacker.getYPosition() - target.getYPosition();
		double distance = Math.sqrt(dx * dx + dy * dy);
		double maxDistance = Math.sqrt(state.getXExtent() * state.getXExtent() + state.getYExtent() * state.getYExtent());
		features[1] = distance / maxDistance;

		// Feature 3: Normalized health of attacker
		features[2] = (double) attacker.getHP() / attacker.getTemplateView().getBaseHealth();

		// Feature 4: Normalized health of target
		features[3] = (double) target.getHP() / target.getTemplateView().getBaseHealth();
		// Feature 5: Normalized number of allies attacking the same target
		int alliesAttackingTarget = 0;
		int lastTurnNumber = state.getTurnNumber() - 1;
		Map<Integer, Action> commandsIssued = history.getCommandsIssued(playernum, lastTurnNumber);
		for (Map.Entry<Integer, Action> commandEntry : commandsIssued.entrySet()) {
			if (commandEntry.getValue() instanceof TargetedAction) {
				TargetedAction action = (TargetedAction) commandEntry.getValue();
				if (action.getTargetId() == tgtUnitId) {
					alliesAttackingTarget++;
				}
			}
		}
		int maxAllies = state.getUnits(playernum).size() - 1; // Excluding the attacker itself
		features[4] = (double) alliesAttackingTarget / maxAllies;

		// Convert features array to a Matrix
		Matrix featureVector = Matrix.full(1, features.length, 0.0);
		for (int i = 0; i < features.length; i++) {
			featureVector.set(0, i, features[i]);
		}
    ...
    private int selectAction(StateView state, HistoryView history, int atkUnitId)
    {
    	...
    	try {
    		...
    		try {
				...
				return selectAction(state, history, atkUnitId);
...
//        double clipValue = 1.0; // Choose an appropriate value based on your problem
//        this.getQFunction().clipGradients(clipValue);
//    	
	...
	public void terminalStep(StateView state, HistoryView history)
	{
		...
		System.out.println(this.getTotalRewards());
    ...
    private void printTestData (List<Double> averageRewards)
    {
        ...
        for (int i = 0; i < averageRewards.size(); i++)
        {
            ...
            String averageReward = String.format("%.9f", averageRewards.get(i));
