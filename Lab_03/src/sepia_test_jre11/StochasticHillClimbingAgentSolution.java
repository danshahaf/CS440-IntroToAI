...
import sepia_test_jre11.SimulatedAnnealingAgentSolution.Coordinate;

import java.io.InputStream;
import java.io.OutputStream;
import java.util.*;

public class StochasticHillClimbingAgentSolution extends Agent {
    ...
    public StochasticHillClimbingAgentSolution(int playernum)
    {
        super(playernum);
        this.footmanID = -1;
        this.townhallID = -1;
    		...
    		int xExtent, int yExtent, Set<Coordinate> obstacles)
    {
    	...
    	float currentUtility = this.heuristic(pt.getXPosition(), pt.getYPosition(), goalPt);

    	// get all uphill moves
    	List<Coordinate> uphillChildren = new ArrayList<Coordinate>(8); // max of 8 children states
    	List<Float> uphillChildrenUtilities = new ArrayList<Float>(8);
    	for(Coordinate child : this.getAndCheckNeighbors(pt, goalPt, xExtent, yExtent, obstacles))
    	{
    		float childUtility = this.heuristic(child.getXPosition(), child.getYPosition(), goalPt);
    		if(childUtility < currentUtility)
    		{
    			uphillChildren.add(child);
    			uphillChildrenUtilities.add(childUtility);
    		}
    	}

    	if(uphillChildren.size() > 0)
    	{
    		// pick and random child...need to convert utilities into probabilities
    		List<Double> childProbabilities = this.utilitiesToProbabilities(uphillChildrenUtilities);
    		nextPosition = this.pickRandomChild(uphillChildren, childProbabilities);
    	} else
    	{
    		this.setIsStuck(true);
    		nextPosition = pt; // dont' go anywhere
    	}
