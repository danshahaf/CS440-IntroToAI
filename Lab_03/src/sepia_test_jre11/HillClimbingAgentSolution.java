...
public class HillClimbingAgentSolution extends Agent {
    ...
    public HillClimbingAgentSolution(int playernum)
    {
        super(playernum);
        this.footmanID = -1;
        this.townhallID = -1;
    ...
    {
    	float current_utility = this.heuristic(pt.getXPosition(), pt.getYPosition(), goalPt);
    	Float best_child_utility = null;
    	Coordinate best_child = null;
    	Coordinate next_position = null;

    	// System.out.println("currently at " + pt);
    	for(Coordinate child : this.getAndCheckNeighbors(pt, goalPt, xExtent, yExtent, obstacles))
    	{
    		// System.out.println("child = " + child);
    		float child_utility = this.heuristic(child.getXPosition(), child.getYPosition(), goalPt);
    		if(best_child_utility == null || child_utility < best_child_utility)
    		{
    			best_child_utility = child_utility;
    			best_child = child;
    		}
    	}
    	if(best_child == null)
    	...
    	{
    		if(best_child_utility < current_utility)
    		{
    			next_position = best_child;
    		} else
    		{
    			next_position = pt;
    			this.setIsStuck(true);
