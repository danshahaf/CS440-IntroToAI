  ...
  */
    ...
    public Map<Integer, Action> initialStep(State.StateView newstate, History.HistoryView statehistory) {
        ...
        for(Integer unitID : enemyUnitIDs) {
            ...
            if(unitType.equals("townhall"))  { townhallID = unitID; }
            else if(unitType.equals("footman")) { enemyFootmanID = unitID; }
            else { System.err.println("Unknown unit type"); }
        }
    ...
    private boolean shouldReplanPath(State.StateView state, History.HistoryView history, Stack<MapLocation> currentPath) { 
        ...
        if (enemyFootmanUnit != null) {
            // --- TRAVERSE THROUGH STACK, CHECK FOR ENEMY'S COORDINATES
            for (MapLocation pathLoc : currentPath) {
                if (enemyFootmanUnit.getXPosition() == pathLoc.x && enemyFootmanUnit.getYPosition() == pathLoc.y) {
                    return true;
                }
            }
        }
     ...
     */
    private Stack<MapLocation> AstarSearch(MapLocation start, final MapLocation goal, int xExtent, int yExtent, MapLocation enemyFootmanLoc, Set<MapLocation> resourceLocations) { // ---- NOTE THAT WE CHANGED goal TO final 
        // -------- SET UP & INITIALIZATION ----------
        PriorityQueue<MapLocation> open = new PriorityQueue<MapLocation>(new Comparator<MapLocation>() {
            public int compare(MapLocation o1, MapLocation o2) { // ---- PRIORITY Q TO COMPARE F SCORE (F = G + H)
                return Float.compare(o1.g + heuristic(o1.x, o1.y, goal), o2.g + heuristic(o2.x, o2.y, goal));
            }
        });
        HashSet<MapLocation> closed = new HashSet<MapLocation>();
        open.add(start); // ----- ADD TO START

        while (!open.isEmpty()) {
            MapLocation rightnow = open.poll(); // ---- SMALLEST F SCORE

            if (rightnow.equals(goal)) { // ------ IF GOAL IS REACHED
                Stack<MapLocation> path = new Stack<MapLocation>();
                return tracePath(start, rightnow, path); // ---- RETURN TRACED PATH (built in function)
            }

            // ---- EXPAND NEIGHBORS
            ArrayList<MapLocation> neighbors = getAndCheckNeighbors(rightnow.f, rightnow, goal, xExtent, yExtent, enemyFootmanLoc, resourceLocations);

            for (MapLocation neighbor : neighbors) { // ---- GO THROUGH NEIGHBORS
                if (closed.contains(neighbor)) { continue; } // ---- SKIP IF VISITED

                float pending_g = rightnow.g + 1; // ----- TENTATIVE G SCORE 

                // ---- IF NOT THERE, ADD NEIGHBOT TO OPEN 
                if (!open.contains(neighbor) || pending_g < neighbor.g) {
                    neighbor.g = pending_g;
                    neighbor.cameFrom = rightnow;
                    open.add(neighbor);
                }
            }

            // ---- MARK CURRENT AS VISITED, ADD TO CLOSE
            closed.add(rightnow);
        }

        // ------ WORST CASE SCENARIO: NO PATH FOUND
        return null;
    }
     ...
     */
    private ArrayList<MapLocation> getAndCheckNeighbors(float parentGPlusEdgeCost, MapLocation pt, MapLocation goal, int xExtent, int yExtent, MapLocation enemy, Set<MapLocation> obstacles) {
    		...
    		for(int i = pt.x - 1; i <= pt.x + 1; i++) { //run from pt.x-1 -> pt.x+1
    			for(int j = pt.y - 1; j <= pt.y + 1; j++) { //run from pt.y-1 -> pt.y+1
    				if(i != pt.x || j != pt.y) { //if we aren't at (pt.x, pt.y)
    					...
    					if(newPt.x >= 0 && newPt.x < xExtent && newPt.y >= 0 && newPt.y < yExtent && !obstacles.contains(newPt)) {
    		...
    		for(int i = pt.x - 1; i <= pt.x + 1; i++) { //run from pt.x-1 -> pt.x+1
    			for(int j = pt.y - 1; j <= pt.y + 1; j++) { //run from pt.y-1 -> pt.y+1
    				if(i != pt.x || j != pt.y) { //if we arent at (pt.x, pt.y)
    					...
    					//if the point is valid (not occupied and in range)
    					if(newPt.x >= 0 && newPt.x < xExtent && newPt.y >= 0 && newPt.y < yExtent &&  !(newPt.x == enemy.x && newPt.y == enemy.y) && !obstacles.contains(newPt)) {
    						neighbors.add(newPt); //add the point to the neighbors list
    					}
    				}
    			}
